...
namespace DMLinter.Analyzers
{
    ...
    public sealed class SpaceAnalyzer : CodeAnalyzer
    {
        ...
        internal override AnalysisResult Analyze(AnalysisContext context)
        {
            ...
            CodeStyleSpaces   spacingStyle = style.Spaces;
            ...
            for (int i = 0; i < tokens.Count; i++)
            {
                ...
                SyntaxToken? next = i + 1 < tokens.Count
                    ? tokens[i + 1]
                    : null;
        ...
        private SyntaxToken CheckSpaces(SyntaxToken token, SyntaxToken? next, bool checkAfter = true, bool shouldBe = false)
        {
            ...
            if (whitespace?.Kind != SyntaxKind.WhiteSpace)
            {
                if (shouldBe)
                {
                    ...
                    {
                        return checkAfter
                            ? CreateTokenWithSpaceAfterFrom(token)
            ...
            else
            {
                if (!shouldBe)
                {
                    ...
                    {
                        return checkAfter
                            ? CreateTokenWithoutSpaceAfterFrom(token)
        ...
        private SyntaxToken CreateTokenWithoutSpaceAfterFrom(SyntaxToken token)
        {
            ...
            newToken.AddTrailTokens(token.Trails.Skip(1)
                                         .ToArray());
        ...
        private SyntaxToken CreateTokenWithoutSpaceBeforeFrom(SyntaxToken token)
        {
            ...
            newToken.AddLeadTokens(token.Leads.SkipLast(1)
                                        .ToArray());

            return newToken;
        }

        private void AddIssue(string id, SyntaxToken token) => _issues.Add(new CodeIssue(id, token, token.Text));
    }
}
